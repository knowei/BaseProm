...
class StartChrome():
    ...
    def create_ppt_doc(self, ppt_dir_path, doc_dir_path):
        ...
        #判断文档类型
        xpath_content = "//div[@class='retype-page']/div/p/img/@data-loading-src|//div[@id='pageNo-1']/div/p/img/@data-src"
    ...
    def create_ppt(self, ppt_dir_path, sel):
        ...
        # 获取标题
        xpath_title =  "//div[@class='doc-title']/span[@class='title']/text()"
        title = "".join(sel.xpath(xpath_title).extract()).strip()
        print(title,'=============')
        # 获取内容
        xpath_content_p = "//div[contains(@id,'pageNo-')]/div/p/img"
        xpath_content_p_list = sel.xpath(xpath_content_p)
        print(xpath_content_p_list,'==============')
    ...
    def create_doc(self, doc_dir_path, sel):
        ...
        # 获取文章内容
        xpath_content = "//div[contains(@id,'pageNo-')]"
        # xpath_content = "//div[contains(@data-id,'div_class_')]/p"
        contents = sel.xpath(xpath_content)
        #print(sel.xpath(xpath_content))
        for content in contents:
            p_txt = ''.join(content.xpath('.//p/text()').extract())
